export default class OAuth2Client {
    ...
    private paths: { auth: string; api: string };

    constructor(config: { bffUrl?: string; paths?: { auth: string; api: string } }) {
    ...
    private loginPopup(): Promise<any> {
        return new Promise((resolve, reject) => {
            const popup = window.open(
                `${this.bffUrl}${this.paths.auth}/login?popup=true`,
                "oauth-login",
    ...
    async callApi(path: string, options: RequestInit = {}) {
        ...
        if (options.body) headers.set("Content-Type", "application/json"); // todo
